...
def add_sale(request):
    if request.method == "POST":
        ...
        purchase = Purchase.objects.create(
            total_amount=data["total_price"], seller=request.user
        )
...
@login_required
def history(request):
    date_range = request.GET.get("date-range", None)
    ...
    last_sales = 0
    for a in Purchase.objects.filter(date__gt=timezone.now() - timedelta(days=7)):
    ...
    return render(
        ...
        "purchase_history.html",
        {
            ...
            "filtered": True if date_range else False,
        },
    )
...
def drug_history(request, drug=None):
    ...
    search_query = request.GET.get("q")
    if search_query:
        getDrug = getDrug.filter(Q(product__name__icontains=search_query))
        for i in getDrug:
